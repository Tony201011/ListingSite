...
class SuburbSearchTest extends TestCase
{
    ...
    public function test_suburb_search_filters_malformed_suburb_and_state_values(): void
    {
        Postcode::query()->create([
            'postcode' => '2000',
            'suburb' => 'null',
        ...
        $response = $this->getJson(route('api.suburbs.search').'?q=Ne');

        $response->assertOk();
        $results = $response->json();
        $labels = array_map(fn (array $item): string => strtolower($item['suburb'].','.$item['state']), $results);

        $this->assertNotContains('null,nsw', $labels);
        $this->assertNotContains('undefined bay,undefined', $labels);
        $this->assertContains('neutral bay,nsw', $labels);
    }

    public function test_suburb_search_normalizes_invalid_postcode_values_to_null(): void
    {
        Postcode::query()->create([
            'postcode' => 'ABCD',
            'suburb' => 'Melbourne',
        ...
        $response = $this->getJson(route('api.suburbs.search').'?q=Me');

        $response->assertOk();
        $results = $response->json();
        $melbourne = collect($results)->firstWhere('suburb', 'Melbourne');
        $this->assertNotNull($melbourne);
        $this->assertNull($melbourne['postcode']);
    }
}

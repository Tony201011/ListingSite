...
class HomeControllerTest extends TestCase
{
    ...
    public function test_favourites_page_resolves_legacy_numeric_favourite_ids_to_slugs(): void
    {
        ...
        $response->assertStatus(200);
        $response->assertViewHas('userFavourites', [(string) $profile->id]);
        $profiles = $response->viewData('profiles');
    ...
    {
        $providerUser = $this->createApprovedProvider(['name' => 'Case Favourite', 'slug' => 'case-favourite']);
        $profile = ProviderProfile::query()->where('user_id', $providerUser->id)->firstOrFail();
        $viewer = User::factory()->create();
    ...
    public function test_favourites_page_resolves_legacy_slug_to_first_profile_with_same_slug(): void
    {
        ...
        $response->assertStatus(200);
        $response->assertViewHas('userFavourites', [$firstProfileId]);
        $profiles = $response->viewData('profiles');
        $this->assertCount(1, $profiles);
        $this->assertSame([$firstProfileId], array_map(fn (array $profile): string => (string) $profile['id'], $profiles));
    }
    ...
    public function test_available_now_filter_shows_only_profiles_with_active_available_now_window(): void
    {
        $userWithAvailableNow = User::factory()->create(['role' => User::ROLE_PROVIDER]);
        $profileWithAvailableNow = $this->createApprovedProfile($userWithAvailableNow, ['name' => 'Available Escort', 'slug' => 'available-escort']);
        $this->createActiveOnlineUser($userWithAvailableNow, $profileWithAvailableNow->id);
        $this->createActiveAvailableNow($userWithAvailableNow, $profileWithAvailableNow->id);

        $userWithoutAvailableNow = User::factory()->create(['role' => User::ROLE_PROVIDER]);
        $profileWithoutAvailableNow = $this->createApprovedProfile($userWithoutAvailableNow, ['name' => 'Online Only Escort', 'slug' => 'online-only-escort']);
        $this->createActiveOnlineUser($userWithoutAvailableNow, $profileWithoutAvailableNow->id);

        $response = $this->get('/?available_now=1');

        $response->assertOk();
        $profiles = collect($response->viewData('profiles')->items())->keyBy('name');
        $this->assertArrayHasKey('Available Escort', $profiles->all());
        $this->assertArrayNotHasKey('Online Only Escort', $profiles->all());
    }

    public function test_available_now_filter_excludes_profiles_with_expired_available_now_window(): void
    {
        $user = User::factory()->create(['role' => User::ROLE_PROVIDER]);
        $profile = $this->createApprovedProfile($user, ['name' => 'Expired Available Escort', 'slug' => 'expired-available-escort']);
        $this->createActiveOnlineUser($user, $profile->id);
        ...
        AvailableNow::query()->create([
            ...
            'usage_date' => today(),
            'usage_count' => 1,
            'available_started_at' => now()->subHours(2),
            'available_expires_at' => now()->subMinutes(5),
        ]);
        ...
        $response->assertOk();
        $profiles = collect($response->viewData('profiles')->items())->keyBy('name');
        $this->assertArrayNotHasKey('Expired Available Escort', $profiles->all());
    }

    public function test_available_now_filter_false_shows_all_online_profiles(): void
    {
        $userWithAvailableNow = User::factory()->create(['role' => User::ROLE_PROVIDER]);
        $profileWithAvailableNow = $this->createApprovedProfile($userWithAvailableNow, ['name' => 'Available Escort', 'slug' => 'available-escort-2']);
        $this->createActiveOnlineUser($userWithAvailableNow, $profileWithAvailableNow->id);
        $this->createActiveAvailableNow($userWithAvailableNow, $profileWithAvailableNow->id);

        $userWithoutAvailableNow = User::factory()->create(['role' => User::ROLE_PROVIDER]);
        $profileWithoutAvailableNow = $this->createApprovedProfile($userWithoutAvailableNow, ['name' => 'Online Only Escort', 'slug' => 'online-only-escort-2']);
        $this->createActiveOnlineUser($userWithoutAvailableNow, $profileWithoutAvailableNow->id);
        ...
        $profiles = collect($response->viewData('profiles')->items())->keyBy('name');
        $this->assertArrayHasKey('Available Escort', $profiles->all());
        $this->assertArrayHasKey('Online Only Escort', $profiles->all());
    }

    public function test_available_now_filter_is_passed_to_view(): void
    {
        $response = $this->get('/?available_now=1');
        $response->assertOk();
        $response->assertViewHas('availableNowFilter', true);

        $response = $this->get('/');
        $response->assertOk();
        $response->assertViewHas('availableNowFilter', false);
    }

    public function test_available_now_filter_works_on_advanced_search(): void
    {
        $userWithAvailableNow = User::factory()->create(['role' => User::ROLE_PROVIDER]);
        $profileWithAvailableNow = $this->createApprovedProfile($userWithAvailableNow, ['name' => 'Available Escort', 'slug' => 'available-escort-3']);
        $this->createActiveOnlineUser($userWithAvailableNow, $profileWithAvailableNow->id);
        $this->createActiveAvailableNow($userWithAvailableNow, $profileWithAvailableNow->id);

        $userWithoutAvailableNow = User::factory()->create(['role' => User::ROLE_PROVIDER]);
        $profileWithoutAvailableNow = $this->createApprovedProfile($userWithoutAvailableNow, ['name' => 'Online Only Escort', 'slug' => 'online-only-escort-3']);
        $this->createActiveOnlineUser($userWithoutAvailableNow, $profileWithoutAvailableNow->id);

        $response = $this->get('/advanced-search?available_now=1');

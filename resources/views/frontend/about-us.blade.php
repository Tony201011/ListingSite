...
@php
    ...
    $pageSubtitle = $page?->subtitle ?: 'Learn why providers and visitors choose our platform and what makes our directory different.';
...
<div class="min-h-screen bg-gray-50" x-data="{}">
    <main class="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
        <div class="min-h-[600px] rounded-lg bg-white p-6 shadow-sm sm:p-8">
            ...
            <p class="mt-2 text-gray-600 {{ $page?->updated_at ? 'mb-2' : 'mb-8' }}">{{ $pageSubtitle }}</p>
            @if($page?->updated_at)
                <p class="mb-8 text-sm text-gray-500">Last updated: {{ $page->updated_at->format('M d, Y') }}</p>
            @endif
            ...
            <div class="border border-gray-300 rounded-lg p-6">
                ...
                @else
                    <p class="text-gray-500">About Us content is not available yet.</p>
                @endif
            </div>
        </div>
    </main>

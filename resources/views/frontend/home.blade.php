...
<div
    x-data="locationPopup({
        maxDistance: {{ $maxSearchDistance }},
        formAction: '{{ url('/') }}',
        locationAlreadyActive: {{ ($userLat !== null && $userLng !== null) ? 'true' : 'false' }}
    })"
    x-show="show"
    x-cloak
...
>
    <div
        ...
        class="relative w-full max-w-sm rounded-2xl bg-white p-6 shadow-2xl text-center"
        @click.stop
    >
        <div class="mb-4 flex justify-center">
            <span class="inline-flex h-14 w-14 items-center justify-center rounded-full bg-pink-100">
                <i class="fa-solid fa-location-dot text-2xl text-pink-600"></i>
            </span>
        </div>
        <h2 id="location-popup-title" class="mb-2 text-lg font-bold text-gray-900">Find escorts near you</h2>
        <p class="mb-1 text-sm text-gray-600">
            Allow location access to see profiles within
            <strong class="text-gray-800">{{ $maxSearchDistance }} km</strong> of your current location.
        </p>
        ...
        <div class="mt-5 flex flex-col gap-2">
            <button
                ...
                @click="allow()"
                :disabled="loading"
                class="w-full rounded-lg bg-pink-600 px-5 py-2.5 text-sm font-semibold text-white transition hover:bg-pink-700 disabled:opacity-60 disabled:cursor-not-allowed"
            >
                <span x-show="!loading">
                    <i class="fa-solid fa-location-crosshairs mr-1.5"></i> Allow location access
                </span>
            ...
            <button
                type="button"
                @click="deny()"
                class="w-full rounded-lg border border-gray-300 bg-white px-5 py-2.5 text-sm font-semibold text-gray-700 transition hover:bg-gray-50"
            >
                No thanks
            </button>
        </div>

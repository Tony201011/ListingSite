@php
        ...
        $useLegacyProfileHeader = request()->routeIs('profile.show');
...
<header class="sticky top-0 z-50 border-b border-gray-800 bg-gray-900/95 backdrop-blur-md" style="{{ $headerStyle }}">
    ...
    <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        <div class="flex min-h-[70px] items-center justify-between gap-4 py-3 {{ $useLegacyProfileHeader ? 'md:hidden' : 'lg:hidden' }}">
            <!-- Logo (escortify) -->
            ...
            <button @click="mobileMenu = !mobileMenu" class="{{ $useLegacyProfileHeader ? 'md:hidden' : 'lg:hidden' }} text-gray-300 hover:text-white" aria-label="Toggle menu">
                <i class="fa-solid fa-bars text-xl"></i>
            </button>
        </div>

        <!-- Second row: main navigation (from first & second images) -->
        <div class="hidden items-center gap-1 border-t border-gray-800 py-3 {{ $useLegacyProfileHeader ? 'md:flex' : 'lg:flex' }}">

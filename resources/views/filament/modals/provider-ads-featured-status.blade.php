    ...
    <div class="overflow-x-auto rounded-lg border border-gray-200 bg-white">
        <table class="min-w-full divide-y divide-gray-200 text-sm">
            ...
            <tbody class="divide-y divide-gray-100 bg-white">
                @foreach ($rows as $row)
                    @php($normalizedStatus = strtolower($row['status']))
                        ...
                        <td class="px-4 py-3">
                            <span
                                @class([
                                    ...
                                    'bg-green-100 text-green-700' => $normalizedStatus === 'active',
                                    'bg-red-100 text-red-700' => in_array($normalizedStatus, ['inactive', 'expired'], true),
                                    'bg-gray-100 text-gray-700' => ! in_array($normalizedStatus, ['active', 'inactive', 'expired'], true),
                                ])
                            >
                                {{ e($row['status']) }}
                            </span>

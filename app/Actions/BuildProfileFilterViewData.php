...
class BuildProfileFilterViewData
{
    ...
    /**
     * Lightweight variant of execute() for paginated AJAX requests.
     *
     * Skips the expensive category filter-group queries and banner profile
     * queries that are only needed for the initial page render. Returns only
     * the profiles paginator so the caller can return a minimal partial view.
     */
    public function executePageOnly(array $validated, bool $syncWithAdminOnlineListing = false, bool $advancedSearch = false): array
    {
        $selectedCategoryIds = collect($validated['categories'] ?? [])
            ->map(fn ($id) => (int) $id)
            ->filter()
        ...
        // Only query category metadata when a category filter is actually active.
        if (! empty($selectedCategoryIds)) {
            $filterSlugs = [
                'hair-color', 'hair-length', 'ethnicity', 'body-type', 'bust-size', 'your-length',
                'primary-identity', 'attributes', 'services-style', 'services-you-provide',

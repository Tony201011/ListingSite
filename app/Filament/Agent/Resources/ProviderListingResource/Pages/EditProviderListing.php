...
class EditProviderListing extends EditRecord
{
    ...
    {
        match (strtolower($this->getCurrentTab())) {
            '1', 'attributes' => $this->updateAttributesTab($record, $data),
            '2', 'contact' => $this->updateContactTab($record, $data),
            '3', 'images' => $this->updateImagesTab($record, $data),
            '4', 'videos' => $this->updateVideosTab($record, $data),
            '5', 'rates' => $this->updateRatesTab($record, $data),
            '6', 'availability' => $this->updateAvailabilityTab($record, $data),
            '7', 'profile message', 'profile-message' => $this->updateProfileMessageTab($record, $data),
            default => $this->updateOverviewTab($record, $data),
        };
    ...
    protected function getCurrentTab(): string
    {
        ...
        // Extract only the label portion so it matches the match expression below.
        if (str_contains($tab, '::')) {

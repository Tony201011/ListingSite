...
final class WooCommerceWebhookController extends Controller
{
    ...
    private function processRefund(array $order, string $purchaseUuid): void
    {
        // Import the refund dependencies inline to keep the constructor lean.
        $walletLedgerService = app(\App\Services\WalletLedgerService::class);

        \Illuminate\Support\Facades\DB::transaction(function () use ($order, $purchaseUuid, $walletLedgerService): void {
            /** @var CreditPurchase|null $purchase */
            $purchase = CreditPurchase::where('uuid', $purchaseUuid)
                ->lockForUpdate()
                ->first();
            ...
            $sourceId = 'refund:'.($order['id'] ?? '');
            $alreadyRefunded = \App\Models\CreditLedgerEntry::where('source_type', 'woocommerce_order')
                ->where('source_id', $sourceId)
                ->exists();
            ...
            \App\Models\CreditLedgerEntry::create([
                'user_id' => $purchase->user_id,
                'credit_purchase_id' => $purchase->id,
                'type' => 'refund',
                'credits_delta' => -$purchase->credits,
            ...
            $profile = \App\Models\ProviderProfile::withTrashed()->find($purchase->provider_profile_id);

            if ($profile instanceof \App\Models\ProviderProfile) {
                $walletLedgerService->record(
                    $profile,
                    -$purchase->credits,
                    'refund',
                    "Refund: -{$purchase->credits} advertising credits for WooCommerce order #{$order['id']}",
